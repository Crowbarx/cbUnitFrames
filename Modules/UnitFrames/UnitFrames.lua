local cb = cbUnitFrames
local E, L, V, P, G = unpack(cb)
...
--assert(UF.oUF, "cbUnitFrames was unable to locate oUF.")
...
function UF:UpdateColors()
	...
	UF.oUF.colors.tapped = E:GetColorTable(db.tapped);
	UF.oUF.colors.disconnected = E:GetColorTable(db.disconnected);
	UF.oUF.colors.health = E:GetColorTable(db.health);
	UF.oUF.colors.power[0] = E:GetColorTable(db.power.MANA);
	UF.oUF.colors.power[1] = E:GetColorTable(db.power.RAGE);
	UF.oUF.colors.power[2] = E:GetColorTable(db.power.FOCUS);
	UF.oUF.colors.power[3] = E:GetColorTable(db.power.ENERGY);

	UF.oUF.colors.ComboPoints = {}
	UF.oUF.colors.ComboPoints[1] = E:GetColorTable(db.classResources.comboPoints[1])
	UF.oUF.colors.ComboPoints[2] = E:GetColorTable(db.classResources.comboPoints[2])
	UF.oUF.colors.ComboPoints[3] = E:GetColorTable(db.classResources.comboPoints[3])
	UF.oUF.colors.ComboPoints[4] = E:GetColorTable(db.classResources.comboPoints[4])
	UF.oUF.colors.ComboPoints[5] = E:GetColorTable(db.classResources.comboPoints[5])

	UF.oUF.colors.reaction[1] = bad
	UF.oUF.colors.reaction[2] = bad
	UF.oUF.colors.reaction[3] = bad
	UF.oUF.colors.reaction[4] = neutral
	UF.oUF.colors.reaction[5] = good
	UF.oUF.colors.reaction[6] = good
	UF.oUF.colors.reaction[7] = good
	UF.oUF.colors.reaction[8] = good
	UF.oUF.colors.smooth = {1, 0, 0,
	1, 1, 0,
	unpack(E:GetColorTable(db.health))}

	UF.oUF.colors.castColor = E:GetColorTable(db.castColor);
end
...
function UF:CreateHeader(parent, groupFilter, overrideName, template, groupName, headerTemplate)
	...
	local db = UF.db.units[group]
	UF:SetActiveStyle("ElvUF_"..E:StringTitle(group))
	local header = UF:SpawnHeader(overrideName, headerTemplate, nil,
...
function UF:CreateAndUpdateHeaderGroup(group, groupFilter, template, headerUpdate, headerTemplate)
	...
	if not self[group] then
		...
		UF:RegisterStyle("ElvUF_"..stringTitle, UF["Construct_"..stringTitle.."Frames"])
		UF:SetActiveStyle("ElvUF_"..stringTitle)
...
function UF:CreateAndUpdateUF(unit)
	...
	if not self[unit] then
		self[unit] = UF:Spawn(unit, "ElvUF_"..frameName)
...
function UF:DisableBlizzard(unit)
...
function UF:Initialize()
	...
	self:UpdateColors()
	UF:RegisterStyle("ElvUF", function(frame, unit)
