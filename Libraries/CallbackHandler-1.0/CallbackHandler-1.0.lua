...
local function CreateDispatcher(argCount)
	...
	local function dispatch(handlers, keys, ...)
		local OLD_ARGS = ARGS
		ARGS = { ... } -- Capture all arguments after handlers and keys
		for i=1, #keys do
			method = handlers[keys[i]]
			if method then
...
function CallbackHandler:New(target, RegisterName, UnregisterName, UnregisterAllName)
	...
	function registry:Fire(eventname, ...)
		...
		local currentKeys = {}
		local currentHandlers = events[eventname]
		for k in pairs(currentHandlers) do
			tinsert(currentKeys, k)
		end
		Dispatchers[tonumber(arg.n) + 1](currentHandlers, currentKeys, eventname, unpack(arg))
